...
class MotionMeanFlow(nn.Module):
    ...
    def p_losses(self, x, cond_frame, cond):
        ...
        # v_theta(z_t,t) := u_theta(z_t, t, t) : imf,    v_theta(=JVP tangent용)
        v_theta = fn(z, t, t, self.cond_drop_prob)


        # build u_target
        if self.use_cfg_target:
            # apply CFG-like target only in a time window
            t_scalar = t.view(b)  # [B]
            ...
            if cfg_mask.any():
                ...
                # v_tilde uses u_cond/u_uncond at (z,t) without grads
                with torch.no_grad():
                    u_cond = fn(z_cfg, r_cfg, t_cfg, drop_prob=0.0)
                    u_uncond = fn(z_cfg, r_cfg, t_cfg, drop_prob=1.0)

                    # SILoss-style mixture:
                    # v_tilde = omega * v + kappa * u_cond + (1-omega-kappa) * u_uncond
                    v_tilde = (
                ...
                def fn_cfg(z_, r_, t_):
                    # derivative of u(z,r,t) w.r.t. t along tangent (v_tilde,0,1)
                    return fn(z_, r_, t_, drop_prob=self.cond_drop_prob)

                # JVP on cfg subset - disable autocast and flash attention for jvp compatibility
                # with torch.autocast(device_type='cuda', enabled=False), \
                #      torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
                #     # Convert to fp32 for jvp
                ...
                #     )
                # u_target_cfg = v_tilde - dt_cfg * dudt_cfg.to(dtype)
                # u_target[idx_cfg] = u_target_cfg
                ...
                with torch.autocast(device_type='cuda', enabled=False), \
                    ...
                    # MF
                    # _, dudt = jvp(fn_std_fp32, (z_fp32, r_fp32, t_fp32), (v_fp32, torch.zeros_like(r_fp32), torch.ones_like(t_fp32)))
                    # IMF: JVP along tangent (v_theta, 0, 1)
                ...
                # IMF: V_pred = u + (t-r) * stopgrad(dudt)
                u = u_fp32.to(dtype)
                dudt = dudt_fp32.to(dtype)
                V_pred = u + time_diff * dudt.detach()
        else:
            # standard MeanFlow target - disable autocast and flash attention for jvp compatibility
            with torch.autocast(device_type='cuda', enabled=False), \
                 torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
                # Convert all inputs to fp32 for jvp
                z_fp32 = z.float()
                r_fp32 = r.float()
                t_fp32 = t.float()
                # v_fp32 = v.float()
        ...
        # MeanFlow regression loss (optionally adaptive weighted)
        # err = u - u_target.detach()
        # Improved MeanFlow regression loss (optionally adaptive weighted)
        err = V_pred - v.detach()
