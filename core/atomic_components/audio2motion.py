...
class Audio2Motion:
    def __init__(
        ...
        ### ================= [수정 1] Scaler 로드 =================
        ### MeanFlow를 사용할 때만 필요한 작업입니다.
        ### lmdm_cfg에 'use_meanflow' 키가 있거나, 그냥 항상 로드해도 무방합니다.
        # self.scaler_path = "/workspace/ditto/ditto-talkinghead-train/checkpoints/stats_lp.pkl" # 경로 확인!
        self.scaler_path = "" # 경로 확인!

        if os.path.exists(self.scaler_path):
            print(f"[Audio2Motion] Loading scaler from {self.scaler_path}")
            stats = joblib.load(self.scaler_path)
            ### numpy 연산을 위해 cpu numpy array로 저장해둡니다.
            self.mean = stats.mean_[:265].astype(np.float32)
            self.scale = stats.scale_[:265].astype(np.float32)
            ...
            ### [로그 추가]
            print(f"[Check] Scale Min: {self.scale.min()}, Max: {self.scale.max()}")
            print(f"[Check] Scale Mean: {self.scale.mean()}")
            
            ### 텐서 연산이 필요할 경우를 대비해 torch tensor도 준비하면 좋지만, 
            ### 여기서는 lmdm 입력 직전에 numpy 처리를 추천합니다.
        else:
            print(f"[WARNING] Scaler not found at {self.scaler_path}!! Result will be broken.")
            self.mean = 0.0
            self.scale = 1.0
        ### =======================================================
    ...
    def setup(
        ...
        # 여기서 kp_source는 "Raw Scale" (큰 값)이다.
        ...
        self.clip_idx = 0
        self.shared_noise = None  # 전체 영상에서 공유할 노이즈 (첫 클립에서 생성)
    ...
    def __call__(self, aud_cond, res_kp_seq=None):
        ...
        # [Step 1] 입력 정규화: 모델에 넣기 전에 작게 만들기
        # self.kp_cond는 현재 Raw Data(큰 값)입니다.
        # (Raw - Mean) / Scale
        norm_kp_cond = (self.kp_cond - self.mean) / self.scale

        # [Step 2] 모델 추론 (shared_noise 전달하여 클립 간 일관성 유지)
        # 정규화된 입력을 넣고, 정규화된 출력을 받습니다.
        norm_pred_kp_seq, used_noise = self.lmdm(norm_kp_cond, aud_cond, self.sampling_timesteps, self.shared_noise)
        
        # 첫 클립에서 생성된 노이즈를 저장하여 이후 클립에서 재사용
        if self.shared_noise is None and used_noise is not None:
            self.shared_noise = used_noise

        # [Step 3] 출력 역정규화: 모델에서 나온 후 다시 크게 만들기
        # 텐서로 나왔을 경우를 대비해 numpy로 변환
        if isinstance(norm_pred_kp_seq, torch.Tensor):
            norm_pred_kp_seq = norm_pred_kp_seq.cpu().numpy()

        # Norm * Scale + Mean
        pred_kp_seq = norm_pred_kp_seq * self.scale + self.mean
