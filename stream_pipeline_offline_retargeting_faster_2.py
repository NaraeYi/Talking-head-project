    ...
    def _trt_render_worker(self):
        ...
        while not self.stop_event.is_set():
            ...
            #     break
                ...
                # 1) GPU putback을 바로 수행 (OpenCV putback_worker로 넘기지 않음)
                # frame_rgb = self.source_info["img_rgb_lst"][frame_idx]  # CPU uint8 RGB (H,W,3)
                # frame_rgb를 GPU 캐시에서 가져오기
                if self.use_gpu_putback and self.preload_bg_to_gpu and "img_rgb_gpu_lst" in self.source_info:
                    frame_rgb = self.source_info["img_rgb_gpu_lst"][frame_idx]  # torch uint8 (H,W,3) on GPU
                else:
                    frame_rgb = self.source_info["img_rgb_lst"][frame_idx]      # numpy uint8 (H,W,3) on CPU
                M_c2o = self.source_info["M_c2o_lst"][frame_idx]        # CPU (2,3) or (3,3)

                t_pb = time.perf_counter()
                res_frame_rgb = self.putback_gpu(frame_rgb, out_t, M_c2o)   # returns CPU uint8 RGB (H,W,3)
                self.timing_stats['putback_total_ms'] += (time.perf_counter() - t_pb) * 1000

                # 2) putback_queue 대신 writer_queue로 바로 넘김
                self.writer_queue.put(res_frame_rgb)
