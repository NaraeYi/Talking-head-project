...
from core.atomic_components.putback_torchGPU import PutBack
from core.atomic_components.putback_torchGPU import PutBackTorchGPU
...
class StreamSDK:
    def __init__(self, cfg_pkl, data_root, use_meanflow, meanflow_mode="improved", **kwargs):
        ...
        self.putback_device = "cuda"
        self.putback_align_corners = True  # OpenCV와 미세 정합 차이 있으면 False도 테스트
        self.preload_bg_to_gpu = True      # VRAM 여유 있으면 True 추천

        self.putback_gpu = PutBackTorchGPU(
            self.putback.mask_ori_float,
            align_corners=self.putback_align_corners,
            device=self.putback_device,
        )

        self.wav2feat = Wav2Feat(**wav2feat_cfg)

        # TRT Renderer / predictor 객체 생성 (FasterLivePortrait: Warp+Decode 통합)
        engine_path = "/workspace/ditto/ditto-talkinghead-train/checkpoints/liveportrait_trt/warping_spade-fix_fp16.engine"
        plugin_path = "/workspace/ditto/ditto-talkinghead-train/checkpoints/liveportrait_onnx/libgrid_sample_3d_plugin.so"

        self.trt_renderer = TensorRTPredictor(
        ...
        self.timing_stats = {
            ...
            'trt_render_total_ms': 0.0,  # total time for trt render (Warp+Decode combined)
            'putback_total_ms': 0.0,  # total time for putback
            'stitch_total_ms': 0.0,  # total time for stitch
            # Retargeting 관련 시간
            'retarget_setup_total_ms': 0.0,  # Cropper 초기화 + lmk_crop 생성 시간
            'retarget_delta_calc_total_ms': 0.0,  # retargeting delta 계산 시간 (첫 계산만)
            'retarget_apply_total_ms': 0.0,  # retargeting delta 적용 시간 (모든 프레임)
            'retarget_delta_calc_count': 0,  # delta 계산 횟수 (보통 1회)
            'retarget_apply_count': 0,  # delta 적용 횟수
        }
    ...
    def setup(self, source_path, output_path, **kwargs):
        ...
        # setup() 또는 source_info 로딩 직후 “배경 프레임 GPU 캐시” 생성
        if self.use_gpu_putback and self.preload_bg_to_gpu:
            img_lst = self.source_info["img_rgb_lst"]  # list of (H,W,3) uint8 RGB
            # VRAM 절약: uint8 (H,W,3) 그대로 GPU에 올려둠 (float 변환은 putback에서 수행)
            self.source_info["img_rgb_gpu_lst"] = [
                torch.from_numpy(np.ascontiguousarray(im)).to(self.putback_device, non_blocking=True)
                for im in img_lst
            ]
        ...
        if self.lp_retarget_enable:
            ...
            if self.lp_retarget_enable:
                if "lmk_crop" not in source_info or source_info["lmk_crop"] is None:
                    retarget_setup_start = time.perf_counter()
                    try:
                        crop_cfg = CropConfig()
                        ...
                        if not osp.exists(crop_cfg.insightface_root) or not osp.exists(crop_cfg.landmark_ckpt_path):
                            ...
                            device_id = 0
                            cropper_init_start = time.perf_counter()
                            self.lp_cropper = Cropper(crop_cfg=crop_cfg, device_id=device_id)
                            cropper_init_time = (time.perf_counter() - cropper_init_start) * 1000

                            img0 = source_info["img_rgb_lst"][0]  # reference RGB
                            crop_start = time.perf_counter()
                            crop_ret = self.lp_cropper.crop_source_image(img0, crop_cfg)  # lmk_crop 생성
                            ...
                            if crop_ret is None:
                                ...
                                retarget_setup_time = (time.perf_counter() - retarget_setup_start) * 1000
                                self.timing_stats['retarget_setup_total_ms'] = retarget_setup_time
                                print(f"[LP-LMK] injected lmk_crop shape={source_info['lmk_crop'].shape}, "
                                    f"cropper_init={cropper_init_time:.1f}ms, crop={crop_time:.1f}ms, "
                                    f"setup_total={retarget_setup_time:.1f}ms")
        ...
        self.trt_render_queue = queue.Queue(maxsize=QUEUE_MAX_SIZE)
        ...
        self.thread_list = [
            ...
            threading.Thread(target=self.trt_render_worker),  # FasterLivePortrait TRT
            # threading.Thread(target=self.putback_worker),
            threading.Thread(target=self.gpu_putback_worker),
            threading.Thread(target=self.writer_worker),
        ]

        # 매 setup마다 타이밍 초기화 (단, retarget_setup_total_ms는 보존)
        retarget_setup_preserved = self.timing_stats.get('retarget_setup_total_ms', 0.0)
        self.timing_stats = {
            'audio2feat_total_ms': 0.0,  # total time for wav2feat (HuBERT)
            'audio2feat_per_step_ms': [],
            'dit_total_ms': 0.0,     # total time for audio2motion
            ...
            'stitch_total_ms': 0.0,  # total time for stitch
            # Retargeting 관련 시간
            'retarget_setup_total_ms': retarget_setup_preserved,  # Cropper 초기화 + lmk_crop 생성 시간 (보존)
            'retarget_delta_calc_total_ms': 0.0,  # retargeting delta 계산 시간 (첫 계산만)
            'retarget_apply_total_ms': 0.0,  # retargeting delta 적용 시간 (모든 프레임)
            'retarget_delta_calc_count': 0,  # delta 계산 횟수 (보통 1회)
            'retarget_apply_count': 0,  # delta 적용 횟수
    ...
    def _writer_worker(self):
        while not self.stop_event.is_set():
            ...
            if item is None:
                break
            frame_idx, out_cpu = item   # out_cpu: (1,3,H,W) uint8 CPU tensor
            frame = out_cpu.numpy()[0].transpose(1, 2, 0)  # (H,W,3) uint8 RGB

            t_start = time.perf_counter()
            self.writer(frame, fmt="rgb")
            t_elapsed = (time.perf_counter() - t_start) * 1000
            self.timing_stats['writer_per_frame_ms'].append(t_elapsed)
            self.timing_stats['writer_total_ms'] += t_elapsed

            self.writer_pbar.update()

    def gpu_putback_worker(self):
        try:
            self._gpu_putback_worker()
        except Exception as e:
            self.worker_exception = e
            self.stop_event.set()

    def _gpu_putback_worker(self):
        while not self.stop_event.is_set():
            try:
            ...
            frame_idx, out_copy = item  # out_copy: (1,3,512,512) GPU float

            # torch.cuda.synchronize()  # trt_render 스레드의 clone() 완료 보장 (크로스-스레드 안전)

            # background frame (가능하면 GPU 캐시 사용)
            if self.use_gpu_putback and self.preload_bg_to_gpu and "img_rgb_gpu_lst" in self.source_info:
                frame_rgb = self.source_info["img_rgb_gpu_lst"][frame_idx]  # torch uint8 (H,W,3) GPU
            else:
                frame_rgb = self.source_info["img_rgb_lst"][frame_idx]      # numpy uint8 CPU

            M_c2o = self.source_info["M_c2o_lst"][frame_idx]  # CPU

            t_pb = time.perf_counter()
            out_u8_gpu = self.putback_gpu(frame_rgb, out_copy, M_c2o, return_gpu_tensor=True)  # (1,3,H,W) uint8 GPU
            self.timing_stats['putback_total_ms'] += (time.perf_counter() - t_pb) * 1000

            out_cpu = out_u8_gpu.to("cpu")
            while not self.stop_event.is_set():
                try:
                    self.writer_queue.put((frame_idx, out_cpu), timeout=1)
                    break
                except queue.Full:
                    continue

    def trt_render_worker(self):
        try:
            self._trt_render_worker()
        except Exception as e:
            self.worker_exception = e
            self.stop_event.set()

    def _trt_render_worker(self):
        frame_idx = None
        while not self.stop_event.is_set():
            try:
                item = self.trt_render_queue.get(timeout=1)
            except queue.Empty:
                continue
            ...
            try:
                ...
                # (1) 입력을 torch CUDA tensor로 맞추기
                if not torch.is_tensor(f_s):
                ...
                f_s = f_s.to("cuda", non_blocking=True).contiguous()
                x_s = x_s.to("cuda", non_blocking=True).contiguous()
                x_d = x_d.to("cuda", non_blocking=True).contiguous()

                # (2) feed_dict 키는 엔진 입력 이름과 일치해야 함
                feed_dict = {
                    "feature_3d": f_s,     # (1,32,16,64,64)
                    "kp_driving": x_d,     # (1,21,3)
                    "kp_source":  x_s,     # (1,21,3)
                }

                # (3) torch current stream을 TRT에 전달
                stream = torch.cuda.current_stream().cuda_stream

                t_start = time.perf_counter()
                tensors = self.trt_renderer.predict(feed_dict, stream)
                self.timing_stats['trt_render_total_ms'] += (time.perf_counter() - t_start) * 1000

                # 출력 텐서 확인
                if "out" not in tensors:
                    available_keys = list(tensors.keys())
                    raise KeyError(f"Output tensor 'out' not found. Available keys: {available_keys}")

            #     # (4) out 텐서를 꺼내서 decode_f3d와 동일 포맷(HWC uint8 RGB)으로 변환
            #     # decode_f3d 출력: (H, W, 3), uint8, [0, 255]
            #     # 하지만 putback의 blend_images_cy는 float32를 기대하므로 float32로 변환
            #     out_t = tensors["out"]  # torch, (1,3,512,512), float
                
            #     # TRT 출력 → render_img 만드는 부분
            #     render_img = (
            #         out_t[0]
            #         .float()
            #         .clamp_(0.0, 1.0)
            ...
            #         .cpu()
            #         .numpy()
            #         .astype(np.float32)  # putback의 blend_images_cy가 float32를 기대
            #     )
            ...
            #     break
                ...
                out_t = tensors["out"]  # (1,3,512,512) torch CUDA float
                out_copy = out_t.detach().clone()  # ★ 버퍼 덮어쓰기 방지 (GPU copy)
                while not self.stop_event.is_set():
                    try:
                        self.putback_queue.put((frame_idx, out_copy), timeout=1)
                        break
                    except queue.Full:
                        continue
    ...
    def _motion_stitch_worker(self):
        while not self.stop_event.is_set():
            ...
            if item is None:
                ...
                self.trt_render_queue.put(None)
                break
            ...
            if getattr(self, "lp_retarget_enable", False):
                retarget_apply_start = time.perf_counter()
                ...
                if w > 0:
                    ...
                    if self.lp_retarget_delta is None:
                        retarget_delta_calc_start = time.perf_counter()
                        ...
                        if ce is not None:
                            ...
                            retarget_delta_calc_time = (time.perf_counter() - retarget_delta_calc_start) * 1000
                            self.timing_stats['retarget_delta_calc_total_ms'] = retarget_delta_calc_time
                            self.timing_stats['retarget_delta_calc_count'] = 1
                    ...
                    if self.lp_retarget_delta is not None:
                        ...
                        if isinstance(x_d, torch.Tensor):
                            ...
                            if self.lp_apply_to == "source":
                                ...
                                x_d = x_d + w * delta_np
                        retarget_apply_time = (time.perf_counter() - retarget_apply_start) * 1000
                        self.timing_stats['retarget_apply_total_ms'] += retarget_apply_time
                        self.timing_stats['retarget_apply_count'] += 1
            # =========================
            
            # self.warp_f3d_queue.put([frame_idx, x_s, x_d])    # ← warping 단계로 넘김 / ⭐️ 여기서 x_s/x_d에 → retargeting Δ를 더해주기
            # self.trt_render_queue.put([frame_idx, x_s, x_d])
            while not self.stop_event.is_set():
                try:
                    self.trt_render_queue.put([frame_idx, x_s, x_d], timeout=1)
                    break
                except queue.Full:
                    continue
