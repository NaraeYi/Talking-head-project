...
class MetricsEvaluator:
    ...
    def _init_calculators(self):
        ...
        try:
            # Require pretrained I3D weights; otherwise FVD numbers are not comparable.
            self.fvd_calc = FVDCalculator(device=self.device, require_pretrained=True)
...
class BatchMetricsEvaluator:
    ...
    def evaluate_batch(self, results_dir: str, test_dir: str,
        ...
        # Collect *paired* video paths for dataset-level FVD.
        # (FVD is defined between two *sets*; in practice you want matching samples.)
        fvd_generated_videos: List[str] = []
        fvd_real_videos: List[str] = []
        ...
        for clip in tqdm(test_clips, desc="Evaluating per-clip metrics"):
            ...
            # We'll always evaluate per-clip metrics on the generated video.
            ...
            if real_videos_dir:
                ...
                if os.path.exists(real_video_path):
                    ...
                    # Only add to the dataset-level FVD pool when BOTH exist.
                    fvd_generated_videos.append(generated_video)
                    fvd_real_videos.append(real_video_path)
        ...
        if real_videos_dir and len(fvd_generated_videos) > 1 and len(fvd_real_videos) > 1:
            try:
                print("\n" + "="*50)
                print("Calculating Dataset-level FVD (standard method)...")
                print("="*50)
                print(f"[FVD] Using {len(fvd_generated_videos)} paired videos")
                fvd_score = self.evaluator.fvd_calc.calculate_dataset_fvd(
                    generated_videos=fvd_generated_videos,
                    real_videos=fvd_real_videos,
                    max_frames=max_frames
                )
            ...
            print("\n[FVD] Skipping dataset-level FVD (need >=2 paired videos and a real_videos_dir)")
...
Examples:
  ...
  python run_all_metrics.py --batch \\
    ...
    # Mode selection
    # NOTE: default=False so single-video mode works when you pass -g/-s.
    ...
    # NOTE: For paper-style FVD, you usually want the *full* clip (or at least the same duration as the paper).
    # Set default=None so we don't silently truncate to 10s.
    parser.add_argument("--max_seconds", type=float, default=None,
                       help="Maximum seconds to process (default: full video). Set e.g. 10.0 for faster evaluation")
